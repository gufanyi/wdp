 ...
 *******************************************************************************/
package xap.lui.compile.ca.jdt.internal.compiler.parser.diagnose;
...
import xap.lui.compile.ca.jdt.internal.compiler.parser.LexStream;
import xap.lui.compile.ca.jdt.internal.compiler.parser.Parser;
import xap.lui.compile.ca.jdt.internal.compiler.parser.ParserBasicInformation;
				...
				while(act > ERROR_ACTION || act < ACCEPT_ACTION) { // SHIFT-REDUCE action or SHIFT action ?
					...
					while(act <= NUM_RULES) {
						...
						do {
							...
							if(DEBUG) {
								CompileLog.debug(Parser.name[Parser.non_terminal_index[lhs_symbol]]);
							}
							tempStackTop -= (Parser.rhs[act]-1);
...
//
	private int parseCheck(int stck[], int stack_top, int first_token, int buffer_position) {
		...
		if (first_token > NT_OFFSET) {
			...
			if(DEBUG_PARSECHECK) {
				CompileLog.debug(String.valueOf(tempStackTop));
			}
			max_pos = stack_top;
			...
			if (act <= NUM_RULES) {
				...
				do {
					...
					if(DEBUG_PARSECHECK) {
						...
						CompileLog.debug(String.valueOf(-(Parser.rhs[act]-1)));
						CompileLog.debug(") [max:"); //$NON-NLS-1$
						CompileLog.debug(String.valueOf(max_pos));
						CompileLog.debug("]\tprocess_non_terminal\t"); //$NON-NLS-1$
						CompileLog.debug(String.valueOf(act));
						CompileLog.debug("\t"); //$NON-NLS-1$
						CompileLog.debug(Parser.name[Parser.non_terminal_index[Parser.lhs[act]]]);
						CompileLog.debug("");
					}
		...
		process_terminal: for (;;) {
			if(DEBUG_PARSECHECK) {
				CompileLog.debug(String.valueOf(tempStackTop + 1));
				CompileLog.debug(" (+1) [max:"); //$NON-NLS-1$
				CompileLog.debug(String.valueOf(max_pos));
				CompileLog.debug("]\tprocess_terminal    \t"); //$NON-NLS-1$
				CompileLog.debug(String.valueOf(ct));
				CompileLog.debug("\t"); //$NON-NLS-1$
				CompileLog.debug(Parser.name[Parser.terminal_index[ct]]);
				CompileLog.debug("");
			}
			...
			if (act <= NUM_RULES) {               // reduce action
				...
				if (act > ERROR_ACTION) {
					...
					if(DEBUG_PARSECHECK) {
						CompileLog.debug(String.valueOf(tempStackTop));
						CompileLog.debug("\tshift reduce"); //$NON-NLS-1$
						CompileLog.debug("");
					}
				} else {
					if(DEBUG_PARSECHECK) {
						CompileLog.debug("\tshift"); //$NON-NLS-1$
					}
