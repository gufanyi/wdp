...
import xap.lui.compile.ca.jdt.core.IJavaElement;
...
import xap.lui.compile.ca.jdt.internal.compiler.lookup.BaseTypeBinding;
import xap.lui.compile.ca.jdt.internal.compiler.lookup.Binding;
import xap.lui.compile.ca.jdt.internal.compiler.lookup.CaptureBinding;
import xap.lui.compile.ca.jdt.internal.compiler.lookup.FieldBinding;
import xap.lui.compile.ca.jdt.internal.compiler.lookup.LocalTypeBinding;
import xap.lui.compile.ca.jdt.internal.compiler.lookup.MethodBinding;
import xap.lui.compile.ca.jdt.internal.compiler.lookup.PackageBinding;
import xap.lui.compile.ca.jdt.internal.compiler.lookup.ParameterizedTypeBinding;
import xap.lui.compile.ca.jdt.internal.compiler.lookup.RawTypeBinding;
...
class TypeBinding implements ITypeBinding {
	...
	xap.lui.compile.ca.jdt.internal.compiler.lookup.TypeBinding binding;
	private String key;
	...
	public TypeBinding(BindingResolver resolver, xap.lui.compile.ca.jdt.internal.compiler.lookup.TypeBinding binding) {
	...
	public IAnnotationBinding[] getAnnotations() {
		...
		if (this.binding.isAnnotationType() || this.binding.isClass() || this.binding.isEnum() || this.binding.isInterface()) {
			xap.lui.compile.ca.jdt.internal.compiler.lookup.ReferenceBinding refType =
				(xap.lui.compile.ca.jdt.internal.compiler.lookup.ReferenceBinding) this.binding;
			xap.lui.compile.ca.jdt.internal.compiler.lookup.AnnotationBinding[] internalAnnotations = refType.getAnnotations();
			int length = internalAnnotations == null ? 0 : internalAnnotations.length;
			if (length != 0) {
				IAnnotationBinding[] tempAnnotations = new IAnnotationBinding[length];
				int convertedAnnotationCount = 0;
				for (int i = 0; i < length; i++) {
					xap.lui.compile.ca.jdt.internal.compiler.lookup.AnnotationBinding internalAnnotation = internalAnnotations[i];
	...
	public String getBinaryName() {
		if (this.binding.isCapture()) {
			...
			xap.lui.compile.ca.jdt.internal.compiler.lookup.Binding declaring = typeVariableBinding.declaringElement;
			StringBuffer binaryName = new StringBuffer();
			switch(declaring.kind()) {
				case xap.lui.compile.ca.jdt.internal.compiler.lookup.Binding.METHOD :
					...
					break;
				default :
					xap.lui.compile.ca.jdt.internal.compiler.lookup.TypeBinding typeBinding = (xap.lui.compile.ca.jdt.internal.compiler.lookup.TypeBinding) declaring;
	...
	public synchronized IMethodBinding[] getDeclaredMethods() {
		...
		try {
			if (isClass() || isInterface() || isEnum()) {
				ReferenceBinding referenceBinding = (ReferenceBinding) this.binding;
				xap.lui.compile.ca.jdt.internal.compiler.lookup.MethodBinding[] internalMethods = referenceBinding.availableMethods(); // be resilient
				int length = internalMethods.length;
				if (length != 0) {
					int convertedMethodCount = 0;
					IMethodBinding[] newMethods = new IMethodBinding[length];
					for (int i = 0; i < length; i++) {
						xap.lui.compile.ca.jdt.internal.compiler.lookup.MethodBinding methodBinding = internalMethods[i];
...
//	}
	...
	/**
	 * @see xap.lui.compile.ca.jdt.core.dom.ITypeBinding#getQualifiedName()
	 */
	...
	public ITypeBinding[] getTypeArguments() {
		...
		if (this.binding.isParameterizedType()) {
			ParameterizedTypeBinding parameterizedTypeBinding = (ParameterizedTypeBinding) this.binding;
			final xap.lui.compile.ca.jdt.internal.compiler.lookup.TypeBinding[] arguments = parameterizedTypeBinding.arguments;
			if (arguments != null) {
	...
	public ITypeBinding[] getTypeBounds() {
		...
		if (this.binding instanceof TypeVariableBinding) {
			...
			xap.lui.compile.ca.jdt.internal.compiler.lookup.TypeBinding firstClassOrArrayBound = typeVariableBinding.firstBound;
			int boundsLength = 0;
	...
	public boolean isCastCompatible(ITypeBinding type) {
		try {
			...
			if (scope == null) return false;
			if (!(type instanceof TypeBinding)) return false;
			xap.lui.compile.ca.jdt.internal.compiler.lookup.TypeBinding expressionType = ((TypeBinding) type).binding;
	...
	public boolean isEqualTo(IBinding other) {
		...
		xap.lui.compile.ca.jdt.internal.compiler.lookup.TypeBinding otherBinding = ((TypeBinding) other).binding;
		// check return type
	...
	public boolean isFromSource() {
		if (isClass() || isInterface() || isEnum()) {
			...
			if (referenceBinding.isRawType()) {
				...
				xap.lui.compile.ca.jdt.internal.compiler.lookup.TypeBinding erasure = parameterizedTypeBinding.erasure();
			...
			if (declaringElement instanceof MethodBinding) {
				...
				final xap.lui.compile.ca.jdt.internal.compiler.lookup.TypeBinding typeBinding = (xap.lui.compile.ca.jdt.internal.compiler.lookup.TypeBinding) declaringElement;
				if (typeBinding instanceof ReferenceBinding) {
					...
					final xap.lui.compile.ca.jdt.internal.compiler.lookup.TypeBinding leafComponentType = arrayBinding.leafComponentType;
	...
	public boolean isNullType() {
		return this.binding == xap.lui.compile.ca.jdt.internal.compiler.lookup.TypeBinding.NULL;
	}
