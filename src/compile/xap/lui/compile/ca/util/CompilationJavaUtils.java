...
import xap.lui.compile.ca.dom.JParam;
...
//						CaLogger.debug("*****Statement内部变量*****");
						CompileLog.debug("statement:"+js.getStatement());
...
//						CaLogger.debug("*****Statement内部方法*****");
						...
						for(int i=jmes.size()-1;i>=0;i--){
							...
							if(jm.getMethodTypeMap().get(jme.getExpression()) == null){	
								...
								//	CompileLog.error(NCLangRes4VoTransl.getNCLangRes().getStrByID("ca", "CompilationJavaUtils-000000")/*当前方法*/+jme.getMethodname()+NCLangRes4VoTransl.getNCLangRes().getStrByID("ca", "CompilationJavaUtils-000001")/*未找到或不存在*/);
								}
							}else{
								Symbol s = new Symbol();
			...
			if(unit != null){
				...
				for(AbstractTypeDeclaration atd:atds){
					...
					if(result == null){//当前类不包含此方法
						...
						//	CompileLog.error(NCLangRes4VoTransl.getNCLangRes().getStrByID("ca", "CompilationJavaUtils-000002")/*方法*/+methodName+NCLangRes4VoTransl.getNCLangRes().getStrByID("ca", "CompilationJavaUtils-000003")/*未在相关类*/+wholeClassName+NCLangRes4VoTransl.getNCLangRes().getStrByID("ca", "CompilationJavaUtils-000004")/*中找到*/);
						}
					}
				}
			}else{
				//CompileLog.error(wholeClassName+NCLangRes4VoTransl.getNCLangRes().getStrByID("ca", "CompilationJavaUtils-000005")/*类文件未找到或不存在*/);
			}
		}
				...
				for(AbstractTypeDeclaration atd:atds){
					...
					if(result == null){//当前类不包含此方法
						if(superClassName != null){//当前类存在父类
							...
							CompileLog.error("方法"+methodName+"未在相关类"+wholeClassName+"中找到");
						}
					}
				}
			}else{
				CompileLog.error(wholeClassName+"类文件未找到或不存在");
			}
		}
