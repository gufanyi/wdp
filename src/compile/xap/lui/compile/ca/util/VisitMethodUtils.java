...
import xap.lui.compile.ca.ast.BlockSymbol;
...
//								jc.getMethodList().add(jm);//去掉重复方法
	...
	private static void visitBodyDeclaration(BodyDeclaration bd ,JProgram jprogram){
		...
		try{
			...
			CompileLog.info("VisitMethodUtils:"+bd.getClass().getSimpleName());
		}catch(Exception e1){CompileLog.info(e1.getMessage());}
		...
		try{
			...
			CompileLog.info("VisitMethodUtils:thrownExceptions:"+bd.getClass().getSimpleName());
		}catch(Exception e1){CompileLog.info(e1.getMessage());}
		
		//MethodDeclaration TypeDeclaration
		try{
			List tps = (List)c.getMethod("typeParameters", null).invoke(bd, null);
			CompileLog.info("VisitMethodUtils:typeParameters:"+bd.getClass().getSimpleName());
		}catch(Exception e1){CompileLog.info(e1.getMessage());}
...
//		LfwLogger.debug(vd.toString());
		...
		try{
			...
			CompileLog.info("VisitMethodUtils:"+exp.toString()+":"+vd.getClass().getSimpleName());
		}catch(Exception e1){CompileLog.info(e1.getMessage());}
	}
}
