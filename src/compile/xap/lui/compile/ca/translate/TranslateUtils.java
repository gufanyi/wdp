 ...
 * 
 */
package xap.lui.compile.ca.translate;
...
public class TranslateUtils {
			...
			switch(s.getType()){
				...
				case Symbol.METHOD_NAME:
					if(judgmentCondition != null){
						if("getWidgetContext".equals(s.getOriginalValue()) && LuiPageContext.class.getName().equals(judgmentCondition.getWholeClassName()) && compareParamTypes(new String[]{"StringLiteral"}, judgmentCondition.getParamTypeNames())){
	...
	public static void translate(String wholeClassName,List<JField> fields,List<JMethod> methods){
		...
		try{
			...
			CompileLog.error(filename + "文件写入失败");
		}finally{
			if(bw != null){
				try {
					bw.close();
				} catch (IOException e) {
					CompileLog.error("写入流关闭失败");
				}
			}
		}
	}
