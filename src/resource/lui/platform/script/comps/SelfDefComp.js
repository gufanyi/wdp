...
function SelfDefComp(parent, name, left, top, width, height, position,
	...
	this.position = $.argumentutils.getString(position, "absolute");
	this.overflow = "hidden";
	this.className = $.argumentutils.getString(className, "self_def_div");
	this.visible = true;
	// 包含自定义对象
...
SelfDefComp.prototype.setContent = function(obj) {
	if (window.editMode != null && window.editMode == true){
		this.Div_gen.appendChild(document.createTextNode("自定义控件"));
		return;
	}
	if (obj != null) {
...
SelfDefComp.prototype.setActive = function(isActive) {
	if (this.contentObj && this.contentObj.setActive)
