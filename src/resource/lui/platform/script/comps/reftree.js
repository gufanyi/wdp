(function($){
	$.reftree={
			...
			$cm_refreshCommand:function(){
				  ...
				  var flag = $.reftree.isFromNc();
				  if(flag){
					  if(ds.reqParameterMap == null)
					  	 ds.reqParameterMap = $.hashmap.getObj();
				...
				for(var i = 0; i < levels.length; i ++){
					...
					for(var j = 0; j < levelArr.length; j++){
						var currentDs = getDataset(levelArr[j].datasetId);
				  		...
				  		if($.reftree.$locateAction1(currentDs, lfs))
				    		break;
					}
				}
			},
			
			$locateAction1:function(ds, lfs){
			...
			otherAction:function(){
				...
				var readDsStr = $.pageutils.getParameter("readDs");
				var readDs = getDataset(readDsStr);
				
				var returnFunc = $.pageutils.getParameter("returnFunc");
				if(returnFunc != null) {
					...
					var writeDsStr = $.pageutils.getParameter("writeDs");
					var readFields = $.pageutils.getParameter("readFields").split(",");
					var writeFields = $.pageutils.getParameter("writeFields").split(",");
					var multiSel = $.pageutils.getParameter("multiSel");
					if(multiSel == null){
