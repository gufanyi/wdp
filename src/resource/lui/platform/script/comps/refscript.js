(function($){
	// 获取打开此参照的父页面
			var trueParent = $.pageutils.getTrueParent();
	$.refscript={
			/**主数据过滤*/
			doFilter:function(value) {
			    var ds = pageUI.getViewPart("main").getDataset("masterDs");
			    if (value != null && value != "") {
			        ds.reqParameterMap.put("filterValue", value);
			        ds.setCurrentPage(0,null,true);
			    } else {
			        var navDs =$.refscript.getRootNavDs();
			        if (navDs != null) { // 树表结构
			...
			/**主数据行选择*/
			selectRow:function(keyCode) {
			    var ds = pageUI.getViewPart("main").getDataset("masterDs");
			    var index = ds.getSelectedIndex();
			    if (null == index) index = -1;
			    if (keyCode == 40) { // 下箭头
			        if (index < ds.getRowCount() - 1) ds.setRowSelected(index + 1);
			    } else if (keyCode == 38) { // 上箭头
			        if (index >= 1) ds.setRowSelected(index - 1);
			        ...
			        if (ds.getAllRowCount() > 0) {
			            ...
			            return false;
			            // 选中操作
			            var grid = pageUI.getViewPart("main").getComponent("refgrid");
			            var tree = pageUI.getViewPart("main").getComponent("reftree");
			            if (grid) {
			...
			/**获取主数据集*/
			getMasterDs:function() {
			    var ds = pageUI.getViewPart("main").getDataset("masterDs");
			    return ds;
			},
			/**从本地数据库加载数据集到目标数据库*/
			doGetFromCache:function(sql, size, keyValue, index, ds, userObj) {
			...
			/**sql替换*/
			aggSql:function(sql, value) {
			    if (sql == null || sql == "") return null;
			    if (value != null && value != "") value = decodeURIComponent(value);
			 ...
			 * 生成参照关联关系SQL语句
			 */
			createRefRelationSql:function(masterFieldInfos) {
			    if (masterFieldInfos != null) {
			        var sql = "";
			        for (var i = 0,
			        n = masterFieldInfos.length; i < n; i++) {
			            ...
			            var widget = trueParent.window.$refNodeRelations.viewpart;
			 ...
			 * 从前台缓存中获取参照数据内容
			 */
			getFromCache:function(userObj) {
			    // 拼接参照关联关系SQL语句
			    var refRelationSql = "";
			    if (trueParent.window.$refNodeRelations != null) {
			        var refNodeId = $.pageutils.getParameter("nodeId");
			        var widget = trueParent.window.$refNodeRelations.viewpart;
			        ...
			        if (refNodeRelation != null) {
			            refRelationSql = $.refscript.createRefRelationSql(refNodeRelation.masterFieldInfos);
			        }
			    }
			    var ds = $.refscript.getMasterDs();
			    // 为Dataset增加参数，供后台查询数据的方法调用
			    if (refRelationSql != null && refRelationSql != "") ds.addReqParameter("relationWhereSql", refRelationSql);
			    return false;
			},
			/**
			*加载一级树数据
			*/
			getRootNavFromCache:function(eventdata) {
			    var userObj = null;
			    if (eventdata != null && eventdata.userObj != null) userObj = eventdata.userObj;
			    var ds = $.refscript.getRootNavDs();
			    var index = 0;
			    var keyValue = ds.reqParameterMap.get($.dataset.CONST.QUERY_PARAM_KEYVALUE);
			    var filterValue = ds.reqParameterMap.get("filterValue");
			    var sql = null;
			    if (filterValue != null) {
			        sql = $.refscript.aggSql($.pageutils.getSessionAttribute("ref_query_sql_root"), filterValue);
			    } else {
			        sql = $.pageutils.getSessionAttribute("ref_query_sql_root");
			        var joinSql = ds.reqParameterMap.get("joinSql");
			        if (joinSql != null) sql += joinSql;
			    }
			    if (sql == null || sql == "") return false;
			
			    return $.refscript.doGetFromCache(sql, -1, keyValue, index, ds, userObj);
			},
			...
			getLeafNavFromCache:function(eventdata) {
			    var userObj = null;
			    if (eventdata != null && eventdata.userObj != null) userObj = eventdata.userObj;
			
			    var ds =$.refscript.getLeafNavDs();
			    var index = 0;
			    var keyValue = ds.reqParameterMap.get($.dataset.CONST.QUERY_PARAM_KEYVALUE);
			    var filterValue = ds.reqParameterMap.get("filterValue");
			    var sql = $.pageutils.getSessionAttribute("ref_query_sql_dir");
			    var joinsql = $.pageutils.getSessionAttribute("ref_query_sql_root_join");
			    if (joinsql) sql = sql + joinsql;
			    if (keyValue != null) {
			        sql = $.refscript.aggSql(sql, keyValue);
			    }
			    if (sql == null || sql == "") return false;
			    ...
			    var ordersql = $.pageutils.getSessionAttribute("ref_query_sql_dir_order");
			    if (ordersql != null && ordersql != "") sql += ordersql;
			    // 为Dataset增加参数，供后台查询数据的方法调用
			    ds.addReqParameter("relationWhereSql", refRelationSql);
			    hasloaddata = $.refscript.doGetFromCache(sql, -1, keyValue, index, ds, userObj);
			    return hasloaddata;
			},
			/**
			* 树的一级行选中后的方法
			*/
			treeRootDsOnAfterRowSelectFromCache:function() {},
			/**
			 * 树表参照，树的二级行选中后执行方法（从缓存中获取表数据）
			 */
			treeLeafDsOnAfterRowSelectFromCache:function(eventdata) {
			    var userObj = null;
			    if (eventdata != null && eventdata.userObj != null) userObj = eventdata.userObj;
			
			    var widget = pageUI.getViewPart("main");
			    var treeDs = $.refscript.getLeafNavDs();
			    ...
			    if (dsRels != null) {
			        ...
			        for (var i = 0; i < masterRels.length; i++) {
			            ...
			            if (keyValue == null) keyValue = selecteRow.rowId;
			            var ds =$.refscript.getMasterDs();
			//            window.lastTreeSelKey = keyValue;
			            ds.reqParameterMap.clear();
			            var joinSql = $.pageutils.getSessionAttribute("ref_query_sql_dir_join");
			            if (joinSql != null) {
			            ...
			            return $.refscript.getFromCache(userObj);
			        }
			    }
			    return false;
			},
			...
			*二级dataset
			*/
			getLeafNavDs:function() {
			    ds = pageUI.getViewPart("main").getDataset("masterDs_tree");
			    if (ds == null) //说明左边没有二级 树，直接加载一级树
			    ds = $.refscript.getRootNavDs();
			    return ds;
			},
			/**
			*一级dataset
			*/
			getRootNavDs:function() {
			    var ds = pageUI.getViewPart("main").getDataset("masterDs_tree_1");
			    return ds;
			},
			 ...
			 * 通过前台缓存处理OK事件（行双击事件）
			 */
			refOkFromCache:function() {
			    if (window.clientMode) {
			        var widgetId = $.pageutils.getParameter("widgetId");
			        var refNodeId = $.pageutils.getParameter("nodeId");
			        var rfnode = trueParent.pageUI.getViewPart(widgetId).getRefNode(refNodeId);
			        var currentDs =$.refscript.getMasterDs();
			        ...
			        if (rfnode.writeDs == null || "" == rfnode.writeDs) { // 未绑定dataset
			            var owner = $.pageutils.getParameter("owner");
			            var refComp = trueParent.pageUI.getViewPart(widgetId).getComponent(owner);
			            if (currRow != null) {
			            ...
			            var wds = trueParent.pageUI.getViewPart(widgetId).getDataset(rfnode.writeDs);
			            ...
			            for (var i = 0; i < writeFields.length; i++) {
			                ...
			                if (currRow != null) {
			                    ...
			                    if (index == -1) {
			                    	$.pageutils.showErrorDialog("没有找到列:" + readFields[i]);
			                        return false;
			                    }
			...
			refClose:function() {
			    ...
			    if (trueParent != null) trueParent.$.pageutils.hideDialog();
			}
	}
})(jQuery)
...
//    var sql = null;
//    if (filterValue != null) {
//        sql = aggSql($.pageutils.getSessionAttribute("ref_query_sql_root"), filterValue);
//    } else {
//        sql = $.pageutils.getSessionAttribute("ref_query_sql_root");
//        var joinSql = ds.reqParameterMap.get("joinSql");
//        if (joinSql != null) sql += joinSql;
...
//李宝亮修改该文件之前 beforeCallServer方法就是被注释掉的 2015年9月22日 11:59:37
//function beforeCallServer(proxy, listenerId, eventName, eleId) {
//    var ds = getMasterDs();
//    if (eleId == ds.id && eventName == "onDataLoad") {
//        // 拼接参照关联关系SQL语句
//        var refRelationSql = "";
//        if (trueParent.window.$refNodeRelations != null) {
//            var refNodeId = $.pageutils.getParameter("nodeId");
