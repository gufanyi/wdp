...
//dingrf 111121 改用 Measures.js中的 $.measures.getZIndex()方法。
 ...
 * @param left 控件左部x坐标
 * @param top 控件顶部y坐标
 * @param width 控件宽度  string类型，值类型为 10px 或 10%
 * @param height 控件高度 string类型，值类型为 10px 或 10%
 */
function BaseComponent(name, left, top, width, height) {	
    ...
    this.id = name;      
    this.left = $.argumentutils.getInteger(left, 0);  
    this.top = $.argumentutils.getInteger(top, 0);        
    this.width = $.argumentutils.getString($.measures.convertWidth(width), '100%');
    this.height = $.argumentutils.getString($.measures.convertHeight(height), '100%'); 
    // 标志控件显示或隐藏的属性       
    this.visible = true;
...
BaseComponent.prototype.setSize = function(width, height) {
	...
	this.height = $.argumentutils.getString(height, "100%");
	if(width != -1)
...
BaseComponent.prototype.destroy = function() {
    ...
    var objHtml = this.getObjHtml();
    if (objHtml){
    	$.pageutils.clearHtmlNodeProperties(objHtml);
    ...
    $.pageutils.clearNodeProperties(this);
};
