...
PrintProxy.prototype.doPrintFile = function(id) {
	var obj = $.pageutils.getSessionAttribute("print_content_" + id);
	if (obj == null) {
		obj = parent.$.pageutils.getSessionAttribute("print_content_" + id);
	}
	this.preview(obj);
};
...
PrintProxy.prototype.printOut = function(printerName, idArr) {
	for ( var i = 0; i < idArr.length; i++) {
		var obj = $.pageutils.getSessionAttribute("print_content_" + idArr[i]);
...
CacheProxy.prototype.initCache = function() {
	$.pageutils.showProgressDialog( trans("rt_00000_1")/*"正在下载客户端缓存,请稍候..."*/);
	var ajax = new Ajax();
...
CacheProxy.prototype.updateCache = function(versionMap) {
	...
	if (tableVersionStr != "") {
		$.pageutils.showProgressDialog(trans("rt_00000_2")/*"正在更新客户端缓存,请稍候..."*/);
		var ajax = new Ajax();
		ajax.setPath(window.globalPath + "/cache");
