...
function flowhResize(e) {
    var oFlow = $(this);
    var id = oFlow.attr('id');

    var newWidth = oFlow.outerWidth();
    //不做判断
    if (true || newWidth != oFlow.oldWidth) {
        var arrKnownWidthItem = [];
        var arrUnknownWidthItem = [];

        var lastChild = null;
        var _flowchildren = oFlow.children();
        for (var i = 0; i < _flowchildren.length; i++) {
            var oFlowItem = $(_flowchildren[i]);
            if (oFlowItem.attr("id") && oFlowItem.attr("id").indexOf(id) != -1) { // 是flowh的子项
                //if (oFlowItem.style.width && oFlowItem.style.width != "") {  // 设置了宽度
                if (oFlowItem.attr("haswidth") == "1") { // 设置了宽度
        ...
        var totalWidth = oFlow.outerWidth();
        for (var i = 0; i < arrKnownWidthItem.length; i++) {
            totalWidth -= arrKnownWidthItem[i].outerWidth();
        }
        ...
        if (itemLength > 0) {
            ...
            for (var i = 0; i < itemLength; i++) {
                var tempWidth = width;
                arrUnknownWidthItem[i].css('width',tempWidth + "px");
                if(arrUnknownWidthItem[i].outerWidth() != tempWidth) {
                	var _finalWidth = tempWidth - (arrUnknownWidthItem[i].outerWidth() - tempWidth);
                	if(_finalWidth<0)
                		_finalWidth = 0;
                	arrUnknownWidthItem[i].css('width',_finalWidth + "px");
                }
                ...
                if (arrUnknownWidthItem[i].outerHeight() == 0 && arrUnknownWidthItem[i].children().length == 0) {
                    arrUnknownWidthItem[i].css('height',oFlow.outerHeight() + "px");
                }
            }
        }
    }
    oFlow.oldWidth = oFlow.outerWidth();
};
