 ...
 * @author licza
 */
public class Add2PanelCmd extends AbstractRaCommand {
	UILayoutPanel uiEle;
	public static Random random = new Random();
	public Add2PanelCmd(RaParameter rp, UIElement uiEle) {
		super(rp);
		this.uiEle = (UILayoutPanel) uiEle;
	}
	@Override
	public void execute() {
		...
		// 如果选择的容器下有控件，则按照用户选择的规则处理
		if (uiEle.getElement() != null) {
			String processRule = checkPanelElement(uiEle);
			// 替换当前元素,先删除在走原添加逻辑
			if (RaCmdConst.A2P_REPLACE.equals(processRule)) {
				rp.setUiId(uiEle.getId());
				rp.setSubuiId(uiEle.getElement().getId());
				CmdInvoker.invoke(new DeleteCmd(rp));
			} else {
				// 上下左右规则的处理
				processCmdWithDirection(processRule, child, uiEle);
				// 需要return
				return;
			}
		}
