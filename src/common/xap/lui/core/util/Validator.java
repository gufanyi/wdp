...
public class Validator {
	...
	public static final String followingMsg = "下列信息";

	public static enum ValidatorType {
		saveFailure, followingMsg, thisFieldCannotNull, thisFieldLengthMoreThanMax, thisFieldValueMoreThanMax, thisFieldValueLessThanMin, thisFieldValueRefuseRule
	}

	public static String getValidatorMsg(int type) {
		String msg = null;
		switch (type) {
		case 0:
			msg = "保存失败！";// NCLangRes4VoTransl.getNCLangRes().getStrByID("bc",
							// "Validator-000001")/*保存失败！*/;
			break;
		case 1:
			msg = "下列信息";// NCLangRes4VoTransl.getNCLangRes().getStrByID("bc",
							// "Validator-000002")/*下列信息*/;
			break;
		case 2:
			msg = "必须输入一个值";// NCLangRes4VoTransl.getNCLangRes().getStrByID("bc",
							// "Validator-000003")/*必须输入一个值*/;
			break;
		case 3:
			msg = "超过了最大长度";// NCLangRes4VoTransl.getNCLangRes().getStrByID("bc",
							// "Validator-000004")/*超过了最大长度*/;
			break;
		case 4:
			msg = "数据超过最大值";// NCLangRes4VoTransl.getNCLangRes().getStrByID("bc",
							// "Validator-000005")/*数据超过最大值*/;
			break;
		case 5:
			msg = "数据低于最小值";// NCLangRes4VoTransl.getNCLangRes().getStrByID("bc",
							// "Validator-000006")/*数据低于最小值*/;
			break;
		case 6:
			msg = "数据不符合校验规则";
			break;
	 ...
	 * @return
	 */
	public static int validtor(Field field, String val) {
		if (val != null)
			val = val.trim();
		int errorType = -1;
		// false-未通过验证
		if (errorType == -1 && !field.isRequire() && (val == null || val.equals(""))) {
			errorType = ValidatorType.thisFieldCannotNull.ordinal();
		}
		if (errorType == -1 && !Validator.maxLengthValid(field, val)) {
			errorType = ValidatorType.thisFieldLengthMoreThanMax.ordinal();
		}
		if (errorType == -1 && !Validator.maxValid(field, val)) {
			errorType = ValidatorType.thisFieldValueMoreThanMax.ordinal();
		}
		if (errorType == -1 && !Validator.minValid(field, val)) {
			errorType = ValidatorType.thisFieldValueLessThanMin.ordinal();
		}
		if (errorType == -1 && !Validator.ruleValid(field, val)) {
			errorType = ValidatorType.thisFieldValueRefuseRule.ordinal();
		}
		return errorType;
	}
	 ...
	 * @return
	 */
	public static boolean maxLengthValid(Field field, String value) {
		String maxLength = (String) field.getExtendAttributeValue(Field.MAX_LENGTH);
		if (null != maxLength && !"".equals(maxLength)) {
			int maxLengthValue = 0;
			try {
				maxLengthValue = Integer.valueOf(maxLength);
			} catch (NumberFormatException e) {
				LuiLogger.error(e);
			}
			if (value != null && maxLengthValue > 0) {
	...
	public static boolean maxValid(Field field, String value) {
		int maxSize = StringUtils.isNumeric(field.getMaxValue()) ? Integer.parseInt(field.getMaxValue()) : 0;
		// 需要验证值
		if (maxSize > 0 && value != null) {
			// 若输入字节长度大于指定长度,则截去超过指定的最大长度的部分
			if (field.getDataType().toLowerCase().endsWith("double") || field.getDataType().toLowerCase().endsWith("float") || field.getDataType().toLowerCase().endsWith("long") || field.getDataType().toLowerCase().indexOf("int") >= 0) {
	...
	public static boolean minValid(Field field, String value) {
		int minSize = StringUtils.isNumeric(field.getMinValue()) ? Integer.parseInt(field.getMinValue()) : 0;
