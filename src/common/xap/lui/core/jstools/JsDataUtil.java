...
//			root.setShowText("待优化CSS组件");
		...
		for (int i = 0; i < dirs.length; i++) {
			...
			if (!f.exists()) {
				//LuiLogger.warn(NCLangRes4VoTransl.getNCLangRes().getStrByID("bc", "JsDataUtil-000000")/*没有找到目录:*/ + dirs[i] + NCLangRes4VoTransl.getNCLangRes().getStrByID("bc", "JsDataUtil-000001")/*,此目录被忽略*/);
				continue;
			} else if (!f.isDirectory()) {
				//LuiLogger.warn(NCLangRes4VoTransl.getNCLangRes().getStrByID("bc", "JsDataUtil-000002")/*指定的路径:*/ + dirs[i] + NCLangRes4VoTransl.getNCLangRes().getStrByID("bc", "JsDataUtil-000003")/*不是目录,此文件被忽略*/);
				continue;
			}
