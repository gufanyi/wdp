...
import xap.lui.core.refrence.AppRefDftOkCmd;
...
public class LuiRefDftCtrl {
	...
	public void onDataLoad(DatasetEvent e) {
		...
		ViewPartMeta parentView = parentPm.getWidget(widgetId);
		GenericRefNode rfnode = (GenericRefNode) parentView.getViewModels().getRefNode(refNodeId);
		BaseRefModel refModel = parentView.getViewModels().getRefModel(rfnode.getRefcode());
		...
		if (ds.getId().equals(IRefConst.MASTER_DS_ID)) {
			...
			if (refType != IRefConst.GRIDTREE) {
				...
				if (null != parameter) {
					...
					if (relationWhereSql != null && !"".equals(relationWhereSql))
						refModel.setWherePart(relationWhereSql);
				}
			} else {
			...
			if(rfnode.getFilterSql()!=null)
				refModel.setWherePart(rfnode.getFilterSql());
	...
	public void searchAllData(MouseEvent e) {
		...
		BaseRefModel refModel = RefSelfUtil.getDefRefModel(rfnode.getRefcode());
		ViewPartContext widgetCtx = AppSession.current().getViewContext();
		ViewPartMeta widget = widgetCtx.getView();
		...
		if (refModel instanceof TreeGridRefModel) {
			((TreeGridRefModel) refModel).setClassJoinValue(IRefConst.QUERY);
		}
		processSelfWherePart(ds, rfnode, filterValue, refModel);
		// 根据参照关联关系产生的查询条件语句
		// 树表的参照加载表时不需要此参数
	 ...
	 */
	protected void processSelfWherePart(Dataset ds, GenericRefNode rfnode, String filterValue, BaseRefModel refModel) {
		...
		if (dsRels != null) {
			...
			for (int i = 0; i < masterRels.length; i++) {
				...
				if (masterSelecteRow != null) { // 有选中行，查询detailDs内容
					...
					PagePartMeta parentPm = LuiRuntimeContext.getWebContext().getParentPageMeta();
					GenericRefNode rfnode = (GenericRefNode) parentPm.getWidget(widgetId).getViewModels().getRefNode(refNodeId);
					BaseRefModel baseRefModel = RefSelfUtil.getDefRefModel(rfnode.getRefcode());
					String pk_org = null;
	...
	public void orgValueChanged(TextEvent e) {
		...
		if (refModel instanceof TreeGridRefModel) {
			...
			TreeRefModel tgModel = (TreeRefModel) refModel;
			if (pk_org != null && !pk_org.equals("")) {
				tgModel.setPk_org(pk_org);
			}
			...
			ds = AppSession.current().getViewContext().getView().getViewModels().getDataset("masterDs");
			GridRefModel ncModel = (GridRefModel) refModel;
			if (pk_org != null && !pk_org.equals("")) {
	...
	protected void processTreeSelWherePart(Dataset ds, GenericRefNode rfnode, BaseRefModel refModel) {}
	/**
	 * 参照初始化后加载参照的值
	 * 
	 * @param pageEvent
	...
	public String[] matchRefPk(ScriptEvent scriptEvent) {
		...
		if (refNode instanceof GenericRefNode) {
			...
			// clearRowValue(ds, rf);
			BaseRefModel rm = RefSelfUtil.getDefRefModel(rf.getRefcode());
			...
			if (null != nameField && nameField.indexOf(".") != -1) {
				String[] fieldCodes = rm.getShowFields().split(",");
	 ...
	 * @param rm
	 */
	protected void onBeforeMatchRefPk(BaseRefModel rm) {}
	/**
	 * 打开参照前获取参数信息
	 * TODO    zhaoweiwu        
	 * @param scriptEvent
...
//	}
	...
	public void matchSearch(ScriptEvent scriptEvent) {
		...
		if (refNode instanceof GenericRefNode) {
			GenericRefNode rf = (GenericRefNode) refNode;
			BaseRefModel ncmodel = RefSelfUtil.getDefRefModel(rf.getRefcode());
			...
			AbstractRefListItem[] objs = new LuiRefDftListFilter().filter(value, ncmodel);
			...
			for (int i = 0; i < objs.length; i++) {
				...
				String refCodeField = ncmodel.getCodeField();
	 ...
	 * @param rm
	 */
	protected void onBeforeMatchSearch(BaseRefModel rm) {}
