 ...
 */
public class LuiRefDftGen {
	// private static final String PARENT_WIDGET = "main";
	private String WIDGET_ID = "main";
	private BaseRefModel model;
	private GenericRefNode refnode;
	public LuiRefDftGen(BaseRefModel model, GenericRefNode refnode) {
		this.model = model;
...
//			gridDs.setPageSize(11);
			// 显示字段列表
			String[] showColumns = treeGridAbsModel.getShowFields().split(",");
			// 不显示字段列表
			String[] hiddenColums = null;
			if(StringUtils.isNotBlank(treeGridAbsModel.getHiddenFields()))
				hiddenColums = treeGridAbsModel.getHiddenFields().split(",");
			ArrayList<String> columnIds = new ArrayList<String>();
			if (showColumns != null) {
				String[] showNames = treeGridAbsModel.getShowFieldNames().split(",");
			...
			if (hiddenColums != null) {
				String[] hiddenNames = treeGridAbsModel.getHiddenFields().split(",");
	...
	private Dataset generateTreeDataset() {
		...
		ds.setEdit(true);
		// 显示字段列表
		String[] showColumns = model.getShowFields().split(",");
		// 不显示字段列表
		String[] hiddenColums = null;
		if(StringUtils.isNotBlank(model.getHiddenFields())) 
			hiddenColums = model.getHiddenFields().split(",");
		if (showColumns != null) {
			for (int i = 0; i < showColumns.length; i++) {
				Field field = new Field();
				String showColumn = showColumns[i];
	 ...
	 * @return
	 */
	private Dataset generateGridDataset(GridRefModel basemodel) {
		if (basemodel == null)
			LuiLogger.error("参照model为空！");
		Dataset ds = new Dataset(IRefConst.MASTER_DS_ID);
//		ds.setPageSize(IRefConst.GRID_PAGESIZE);
		ds.setPageSize(basemodel.getPageSize());
		...
		// 显示字段列表
		String[] showColumns = basemodel.getShowFields().split(",");
		// 不显示字段列表
		String[] hiddenColums = null;
		if(StringUtils.isNotBlank(basemodel.getHiddenFields()))
			hiddenColums = basemodel.getHiddenFields().split(",");
		if (showColumns != null) {
			String[] showNames = basemodel.getShowFieldNames().split(",");
			for (int i = 0; i < showColumns.length; i++) {
				String showColumn = showColumns[i];
				Field field = new Field();
				String text = null;
	...
	public WebComp[] getComponent() {
		...
		if (refType == IRefConst.GRID) {
			return new GridComp[] {
				generateGrid(IRefConst.MASTER_DS_ID)
			};
	...
	public String[] getRefElements() {
		String refPk = model.getPkField();
		if (refPk != null) {
			if (refPk.indexOf(".") != -1)
				refPk = refPk.split("\\.")[1];
		}
		String code = model.getCodeField();
		if (code != null) {
			if (code.indexOf(".") != -1) {
				code = code.split("\\.")[1];
			}
		}
		String name = model.getNameField();
		return new String[] {
				refPk, code, name
		};
	}
	public String[] getRefElementsNoTrans() {
		return new String[] {
				model.getPkField(), model.getCodeField(), model.getNameField()
		};
	}
	/**
	 * 获取客户端用户设置column宽度
	...
	private GridComp generateGrid(String dsId) {
		// 显示字段列表
		String[] showColumns = model.getShowFields().split(",");
		String[] nameColumns = ((GridRefModel) model).getShowFieldNames().split(",");
	...
	private TreeViewComp generateTgTree(String dsId) {
		...
		if (classChildField == null) {
			...
			if (refType == IRefConst.TREE) {
				level.setMasterKeyField(treeModel.getPkField());
				level.setLabelFields(treeModel.getCodeField().replaceAll("\\.", "_") + "," + treeModel.getNameField().replaceAll("\\.", "_"));
			} else if (refType == IRefConst.GRIDTREE) {
				String classRefPkField = treeModel.getClassPKField();
				...
				String classRefCode = treeModel.getClassCodeField();
				String classRefNameField = treeModel.getClassNameField();
		...
		// 递归树
			...
			if (refType == IRefConst.TREE) {
				level.setMasterKeyField(treeModel.getPkField());
				String code = treeModel.getCodeField();
				String name = treeModel.getNameField();
				String labelFields = "";
				...
				level.setMasterKeyField(treeModel.getClassPKField());
				// level.setMasterKeyField(treeModel.getClassChildField());
				String code = treeModel.getClassCodeField();
				String name = treeModel.getClassNameField();
				String labelFields = "";
				if (code != null && !"".equals(code))
					labelFields = code.replaceAll("\\.", "_") + "," + name.replaceAll("\\.", "_");
				else
	...
	private WebComp[] generateTree(String dsId) {
		boolean isGridTree = (RefSelfUtil.getRefType(model) == IRefConst.GRIDTREE);
		...
		if (treeModel.getChildField() == null || treeModel.getFatherField() == null) {
			...
			level.setId("level1");
			level.setDataset(dsId);
			String pkField = treeModel.getPkField();
			if (pkField.indexOf(".") != -1) {
				level.setMasterKeyField(pkField.split("\\.")[1].replaceAll("\\.", "_"));
			} else {
				level.setMasterKeyField(pkField.replaceAll("\\.", "_"));
			}
			String code = treeModel.getCodeField().indexOf(".") != -1 ? treeModel.getCodeField().split("\\.")[1].replaceAll("\\.", "_") : treeModel.getCodeField().replaceAll("\\.", "_");
			String name = treeModel.getNameField().indexOf(".") != -1 ? treeModel.getNameField().split("\\.")[1].replaceAll("\\.", "_") : treeModel.getNameField().replaceAll("\\.", "_");
		...
		if (treeModel.getChildField() != null || treeModel.getFatherField() != null) {
			...
			String refCodeField = treeModel.getCodeField();
			String refNameField = treeModel.getNameField();
