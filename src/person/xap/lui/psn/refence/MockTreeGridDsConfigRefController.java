...
public class MockTreeGridDsConfigRefController extends MockTreeGridController {
	...
	public void dataLoad(DatasetEvent e) {
		...
		try {
			ModuleDO[] moduleList = MetaDataPool.getModules();
			if (moduleList != null && moduleList.length > 0) {
				for (ModuleDO vo : moduleList) {
					...
					ComponentDO[] list = MetaDataPool.getComponentDos(vo.getId());

					for (ComponentDO inner : list) {
						Row empRow1 = ds.getEmptyRow();
						empRow1.setValue(idIndex, inner.getId());
						empRow1.setValue(pIdIndex, inner.getModuleid());
						empRow1.setValue(labelIndex, inner.getDisplayname());
						ds.addRow(empRow1);
					}
				}
			}
	...
	public void okButtonClick(MouseEvent e) {
		...
		if (StringUtils.equals(writeDs, "dscfgds")) {
			...
			Dataset ds = LuiAppUtil.getCntAppCtx().getWindowContext("dscfg").getViewContext("main").getView().getViewModels().getDataset("classds");

			try {
				PropertyDO[] dos = MetaDataPool.getPropertyDos(id);
				ds.setEdit(false);
				...
				try {
					...
					for (PropertyDO pro : dos) {
						if (pro.getAccessorclassname() != null) {
							ClassDO[] classlist = MetaDataPool.getClassDos(pro.getDatatype(), null);
	...
	public void afterRowSel(xap.lui.core.event.DatasetEvent e) {
		...
		if (METADATA.equals(id) || METADATA.equals(pid))
			...
			// String sql = " id = '"+id+"'";
			try {
				ComponentDO[] complist = MetaDataPool.getComponentDos(id);
				if (complist != null && complist.length > 0) {
					for (ComponentDO comp : complist) {
						ClassDO[] classlist = MetaDataPool.getClassDos(comp.getId(), "201");
	...
	public void textValueChanged(TextEvent e) {
		...
		ds.addRow(metaDataRow);
		try {
			ModuleDO[] moduleList = MetaDataPool.getLikeModule(comp.getValue());
			if (moduleList != null && moduleList.length > 0) {
				for (ModuleDO vo : moduleList) {
					Row empRow = ds.getEmptyRow();
					empRow.setValue(idIndex, vo.getId());
			...
			ComponentDO[] list = MetaDataPool.getLikeComponentDos(comp.getValue());
