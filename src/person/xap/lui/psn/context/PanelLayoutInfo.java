...
public class PanelLayoutInfo extends LayoutInfo {
	...
	public PanelLayoutInfo(){
		...
		renderType.setDsField("string_ext3");
		renderType.setLabel("渲染类型");
		renderType.setVoField("rendertype");
		list.add(renderType);		
		
		ComboPropertyInfo expand = new ComboPropertyInfo();
		expand.setId("expand");
		expand.setType(StringDataTypeConst.BOOLEAN);
		expand.setVisible(true);
		expand.setEditable(true);
		expand.setKeys(new String[]{"是","否"}); 
		//UIConstant中true为0
		expand.setValues(new String[]{"Y", "N"});
		expand.setDsField("combo_ext4");
		expand.setLabel("默认展开");
		expand.setVoField("expand");
		list.add(expand);		
